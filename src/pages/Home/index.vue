<template>
  <el-container class="honmeIndex">
    <el-header style="top: 697px;">
      <home-header style="margin:0 auto;width: 75%"></home-header>
    </el-header>
                 ...
                 <div class="numtop">
                   <div class="numtoplis" v-for="(item,index) in numtoplis" :key="index" v-if="index<4">
                     ...
                     <div class="num">{{ item.nowPrice }}<span class="iconfont"
                                                               :class="item.increaseRate>0?'icon-direction-top icon-direction-top_red':'icon-direction-bottom icon-direction-bottom_green'"></span>
                     </div>
    ...
    <div class="contentIndex">
      ...
      <div class="realTime">
        <ul>
          <li v-for="(item,index) in numtoplis" :key="index" v-if="index<6" :class="item.increaseRate>0?'red': 'green'">
            <p>{{ item.nowPrice }}</p>
            <em>{{ item.name }}</em>
            <span>{{ item.increase }}</span>
            <span>{{ item.increaseRate }}%</span>
          </li>
        </ul>
      </div>
      <div class="news-box">
        <div class="news-box-left">
          <h3>
            ...
            <span @click="gotoNewsMore">查看更多</span>
          </h3>
          <ul>
            <li v-for="(item,index) in noticeList" :key="index">
              <h4 @click="gotoNoticeDetail(item.id)">{{ item.title }}</h4>
              <p>
                <span>{{ (new Date(item.addTime)).getFullYear() }}.{{ (new Date(item.addTime)).getMonth() }}.{{ (new Date(item.addTime)).getDate() }}</span>
                <span>|</span>
        ...
        <div class="news-box-right">
          ...
          <div class="newsFlsh-con">
            <ul>
              <li v-for="(item,index) in ArtList" :key="index">
                <span class="time"> {{ item.addTime }}</span>
                <p  @click="gotoNoticeDetail(item.id)">{{ item.artTitle }}</p>
              </li>
            </ul>
          </div>
        </div>
...
<style lang="less" scoped>
  ...
  .realTime ul li.red p, .realTime ul li.red span {
    color: #f14957;
  }

  .realTime ul li.green p, .realTime ul li.green span {
    color: #12a875;
  }

  .realTime ul li em, .realTime ul li span {
    font-size: 14px;
    margin-right: 10px;
  }
